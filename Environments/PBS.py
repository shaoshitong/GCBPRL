...
class EnvironmentPBS(gym.Env):
    ...
    def __init__(self, queue_number, path):
        ...
        self.observation_space = spaces.Box(  # TODO: (7 , 10) [PBS的状态]
            low=0, high=1, shape=(7, 10), dtype=np.int32
        )  # TODO: 假设第七行是返回道
        self.observation_space_2 = spaces.Box(
            low=np.array([0, 0]), high=np.array([1, 1]), dtype=np.int32
        )
        self.observation_space_3 = spaces.Box(
        ...
        self.reset_number = 0
        self.reward_result = []
        self.begin_obs, self.begin_reward, self.begin_done, self.begin_choose_action = self.reset(
            path
        )
    ...
    def _next_observation(self):  # TODO: get observation
        return (
            ...
            copy.deepcopy(self.agent_observation),
            self.tuzhuang_observation,
        )
    ...
    def reset(self, path: str):  # TODO: 环境重置
        ...
        self.tuzhuang_observation = [
            Queue(maxsize=self.queue_number + 1),
            Queue(maxsize=self.queue_number + 1),
        ]
        ...
        self.tuzhuang_observation_require_time = np.zeros(
            (2,), dtype=np.int32
        )  # TODO: (3) [涂装口和总装口]
        self.current_step = 0
        self.output_array = []
        ...
        obs_time_1, obs_time_2, obs_time_3 = (
            self.observation_require_time,
            self.agent_observation_require_time,
            self.tuzhuang_observation_require_time,
        )
    ...
    def _every_time_step(self):  # TODO: 环境的单步更新
        ...
        now_tuzhuang_observation_require_time = copy.deepcopy(
            self.tuzhuang_observation_require_time
        )
        ...
        now_tuzhuang_observation_require_time = np.clip(
            now_tuzhuang_observation_require_time - 1, 0, 1e6
        )
        ...
        if agent_1_require_time == 0:
            if 0 < now_agent_observation[0] <= 6 and (
                ...
                now_observation_require_time[
                    now_agent_observation[0] - 1, 0
                ] = self.agent_1_require_time_1[now_agent_observation[0] - 1]
        ...
        for i in range(9, 0, -1):
            ...
            if (~mask).sum().item() > 0:
                temp = np.where(
                    0 < now_observation_require_time[:6, i] - 1,
                    now_observation_require_time[:6, i] - 1,
                    np.zeros_like(now_observation_require_time[:6, i]),
                )
                now_observation_require_time[:6, i] = np.where(
                    mask, now_observation_require_time[:6, i], temp
                )
        ...
        if f_mask.sum().item():
            now_observation_require_time[:6, 0][f_mask] = np.where(
                now_observation_require_time[:6, 0][f_mask] > 0,
        ...
        if f_mask:
            now_observation_require_time[6, 9] = np.where(
                now_observation_require_time[6, 9] > 0,
                now_observation_require_time[6, 9] - 1,
                now_observation_require_time[6, 9],
            )

        self.observation, self.agent_observation, self.tuzhuang_observation = (
            now_observation,
            now_agent_observation,
            now_tuzhuang_observation,
        )
        (
            self.observation_require_time,
            self.agent_observation_require_time,
            ...
            now_observation_require_time,
            now_agent_observation_require_time,
            now_tuzhuang_observation_require_time,
        )
    ...
    def _every_car_where(self):
        ...
        if self.agent_observation_require_time[0] > 0:
            index = (
                self.agent_observation[0] - 7
                if self.agent_observation[0] >= 7
                else self.agent_observation[0] - 1
            )
    ...
    def _reward_compute(self, done):  # TODO: 计算当前回报 (相较于上一次回报获取，增加或者减少了多少)
        ...
        if done:
            ...
            self.reward_result.append(
                [
                    self.last_constraint_1 * 0.4,
                    self.last_constraint_2 * 0.3,
                    self.last_constraint_3 * 0.2,
